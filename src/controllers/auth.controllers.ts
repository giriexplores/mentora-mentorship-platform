...
const signupSchema = insertUserSchema.pick({name:true, email: true, password: true, role: true }).extend({
  name: z.string(),
  email: z.email(),
...
export async function signup(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if(password!==confirmPassword){
      sendError(res, "password and confirmPassword do not match", 409)
      return;
    }
